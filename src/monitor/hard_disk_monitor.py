...
from src.base.monitor import MonitorBase
from src.config import settings, user_info
from src.entity import hard_disk
from src.utils import common_utils, config_utils, logs, system
...
class HardDiskMonitor(MonitorBase):
    ...
    def update_disk_detail_info(self) -> None:
        ...
        """
        command = "df -lh"
        _, results, _ = common_utils.get_terminal_output(command)
    ...
    def hard_disk_monitor_thread(self) -> None:
        ...
        while self.monitor_thread_work:
            ...
            for mount_point, disk in self.hard_disk_dict.items():
                ...
                if not config_utils.is_webhook_quiet_period():
                    ...
                    message_handler.enqueue_hard_disk_warning_msg(disk.disk_info)
    ...
    def get_user_dir_size_info(self, disk: hard_disk.HardDisk) -> None:
        ...
        while retry_count < 5:
            try:
                result_code, results, _ = common_utils.get_terminal_output(command_args)
                if result_code == 0:
                    break  # 命令执行成功，退出循环
    ...
    def get_machine_hard_disk_dict(self) -> dict[str, str]:
        ...
        results = common_utils.read_file(Path("/proc/mounts"))
    ...
    @staticmethod
    def parse_dir_size_info(
        detail_dirs_info: list[str], disk: hard_disk.HardDisk
    ) -> None:
        """
        ...
        for lines in detail_dirs_info:
            ...
            message_handler.enqueue_hard_disk_warning_msg_to_user(
