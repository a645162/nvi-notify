...
from src.config import user_info
from src.utils import config_utils, logs, process_utils
from src.webhook import enums as webhook_enums
...
class EnvironmentManager:
    ...
    def load_env(cls) -> None:
        ...
        debug_mode = process_utils.is_debug_mode()
        if debug_mode:
    ...
    @staticmethod
    def check_python_version(min_required_version: str) -> None:
        """
        ...
        """
        current_version = EnvironmentManager.get_python_version()

        if version.parse(current_version) < version.parse(min_required_version):
            msg = "Python version must be greater than or equal to {}".format(  # noqa: UP032
...
EnvironmentManager.check_python_version("3.10")
PYTHON_VERSION = EnvironmentManager.get_python_version()
...
if USE_GROUP_CENTER and ENV_FROM_GROUP_CENTER:
    from src.config import remote as center_config

    center_config.load_remote_config()
...
WEBHOOK_NAME = set(
    m.strip().upper()
    for m in EnvironmentManager.get(
        "WEBHOOK_NAME", webhook_enums.AllWebhookName.WEWORK.value
    ).split(",")
)
