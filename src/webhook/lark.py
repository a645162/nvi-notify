...
class LarkWebhook(webhook.Webhook):
    ...
    def send_message(
        ...
        if msg_type != enums.MsgType.WARNING:
            # send msg to user by lark app
            self.send_lark_message_by_app(msg, msg_type, user)
            if msg_type == enums.MsgType.DISK_WARNING_TO_USER:
    ...
    def get_group_msg_mention_header(
    ...
    def send_lark_message_by_app(
        ...
        if (
            ...
            or user is None
            or msg_type == enums.MsgType.WARNING
        ):
            return
