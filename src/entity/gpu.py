...
import nvitop
from nvitop.api import process as nvitop_process

from src.api import data_manager
from src.config import settings
from src.database import sqlite
from src.entity import process, task_info
from src.monitor import enums
from src.utils import data_utils, logs
from src.webhook import message_handler
...
class GPU:
    def __init__(self, gpu_id: int, multi_gpu_machine_flag: bool) -> None:
        ...
        self.processes: dict = {}
        self.nvidia_i: nvitop.Device = nvitop.Device(self.gpu_id)
    ...
    def handle_death_processes(self) -> None:
        ...
        for pid in tmp_process:
            ...
            self.num_task -= 1
            self.processes[pid].state = enums.TaskState.DEATH
    ...
    def update_new_processes_info(self) -> None:
        ...
        for pid, _gpu_process in self.all_processes.items():
            ...
            if (
                ...
                new_process.state = enums.TaskState.WORKING
            else:
                new_process.state = enums.TaskState.NEWBORN
    ...
    @property
    def all_processes(self) -> dict[int, nvitop_process.GpuProcess] | None:
        try:
            return self.nvidia_i.processes()
        except Exception as e:
            logger.error(e)
            message_handler.enqueue_except_warning_msg("process")
    ...
    def get_gpu_tasks_num_msg_header(self) -> None:
        if self.num_task == 0:
            ...
            self.gpu_tasks_num_msg_header = (
                f"{task_info.TaskInfoForWebhook.get_emoji('呲牙') * self.num_task}"
    ...
    def get_all_tasks_msg_body_for_task_finish(self, finished_pid: int) -> None:
        ...
        all_tasks_msg_list = []
        task_idx = 0
        for pid, _process in self.processes.items():
            if finished_pid == pid:
                continue
            all_tasks_msg_list.append(self.gen_task_msg_lite(task_idx, _process))
            task_idx += 1

        self.all_tasks_msg_body = "".join(all_tasks_msg_list)

    @staticmethod
    def gen_task_msg_lite(task_idx: int, process: process.GPUProcessInfo) -> str:
        idx_emoji = task_info.TaskInfoForWebhook.get_emoji(task_idx)
            ...
            data_manager_ins.gpu_usage[self.gpu_id].update(
                {
                    ...
                    "gpuMemoryTotalMB": data_utils.Converter.convert_bytes_to_mb(
                        self.memory_total
                    ),
